...
(function() {
  ...
  tool = {
    ...
    copy: function(origin, target) {
      var f, files, l, len, oCur, results, s, tCur;
      if (existsSync(origin)) {
        ...
        if (files) {
          ...
          for (l = 0, len = files.length; l < len; l++) {
            ...
            s = fs.statSync(oCur);
            if (s.isFile()) {
              results.push(fs.writeFileSync(tCur, fs.readFileSync(oCur, ''), ''));
            } else {
              if (s.isDirectory()) {
                results.push(tool.copy(oCur, tCur));
              } else {
                results.push(void 0);
              }
            }
